...
- S.1 (estabilización v0.2): el exit status de smartctl es un BITMASK de 8 bits (bits 0-2 operativos, 3-7 hallazgos que coexisten con JSON utilizable); `smartctl.exit_status` JSON es numérico; el transporte `-d TYPE` (device.Type: scsi/nvme/sat/sntjmicron...) se preserva en análisis y self-tests; `ApproximateDiskType` (clasificación visual) nunca se usa como -d; `SmartctlDeviceType` persiste en reportes/sesiones con fallback a autodetección
- S.2 (estabilización v0.2): semántica de salud SMART — Good solo con evidencia positiva (smart_status.passed=true); RawValue nunca se compara con THRESH (aplica al valor normalizado); when_failed/flags.prefailure como señales del estándar; política CATTECH por ID (5/197/198) documentada como conservadora, no universal; CRC ID199 = warning de interfaz; SSD vendor-specific sin thresholds raw; temperatura vía temperature.current; NVMe critical_warning numérico, media_errors >0 crítico, unsafe_shutdowns informativo; backup solo por señales críticas reales
